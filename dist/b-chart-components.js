...
(function () {
    ...
    var BLineChartPrototype = Object.create(HTMLElement.prototype, {
            width: {
                enumerable: true,
                get: function () {
                    return this.hasAttribute('width') ? parseInt(this.getAttribute('width')) : this.getBoundingClientRect().width;
                }
            },
            data: {
                enumerable: true,
                set: function (data) {
                    ...
                    this.svg.append('g').attr('class', 'y axis').call(yAxis).append('text').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '.71em').style('text-anchor', 'end').text('Price ($)');
                    this.svg.append('path').datum(data).attr('class', 'line').attr('d', line);
                }
            },
            createdCallback: {
                enumerable: true,
                value: function () {
                    this.svg = d3.select(this).append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
                }
            }
        });
...
(function () {
    ...
    window.BPieChart = document.registerElement('b-pie-chart', { prototype: BPieChartPrototype });
}());